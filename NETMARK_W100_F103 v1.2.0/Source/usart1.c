...
void Usart1GetCommand(void)  //串口1接收
{
				...
				if(charging_flag == on)
				{
					if(code_flag2==on)  //加密情况
					{
						if(code_flag1==on)  //密码正确
						{
								...
								WriteflashBoatnum();
								ReadflashBoatnum();
								com3_txbuf[0] = '$'; com3_txbuf[1] = 0x01;
								for(i=0;i<16;i++)
									com3_txbuf[i+2] = com1_rxbuf[i+2];
								sendtocom3();
	...
	//				time_o = 0xffff;
					...
					com3_txbuf[0] = '$'; com3_txbuf[1]=0x0C; com3_txbuf[2]=0x01;  //成功恢复出厂设置
					for(i=3;i<18;i++)
						com3_txbuf[i]=0x00;
		...
		// 			com3_txbuf[0] = '$'; com3_txbuf[1]=0x12; com3_txbuf[2]=com1_rxbuf[4]; com3_txbuf[3]=com1_rxbuf[5];
		// 			for(i=4;i<18;i++)
		// 				com3_txbuf[i]=0x00;
					for(i=0;i<18;i++)
						com3_txbuf[i] = com1_rxbuf[i];
			...
			case 0x17: //连接网位仪
			{
				...
				com3_txbuf[0] = '$'; com3_txbuf[1] = 0x17;
				for(i=2;i<18;i++)   com3_txbuf[i]=0x00;
				sendtocom3();
			...
			case 0x18: //断开网位仪
			{
				com3_txbuf[0] = '$'; com3_txbuf[1] = 0x18;
				for(i=2;i<18;i++)   com3_txbuf[i]=0x00;
				sendtocom3();
