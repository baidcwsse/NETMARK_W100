...
void L01_SetTXMode( void )
{
    ...
    L01_WriteSingleReg( L01REG_CONFIG, controlreg & ~( 1<<PRIM_RX ) | ( 1<<PWR_UP ) );
}
/*
================================================================================
...
//关闭无线,用于省电
void L01_SetOFFMode( void )
{
    INT8U controlreg = L01_ReadSingleReg( L01REG_CONFIG );
    L01_WriteSingleReg( L01REG_CONFIG, controlreg & ( 0<<PWR_UP ));
}
/*
================================================================================
Function : L01_Init( )
...
// #endif//DYNAMIC_PACKET
    ...
    L01_WriteSingleReg( L01REG_EN_AA, ( 1<<ENAA_P0 ) );   //Enable Auto ack in pipe 0
     L01_WriteSingleReg( L01REG_EN_AA, ( 0<<ENAA_P0 ) );   //Disable Auto ack in pipe 0
//    L01_WriteSingleReg( L01REG_EN_RXADDR, ( 1<<ERX_P0 ) );//Enable pipe 0 receive
    ...
    L01_WriteSingleReg( L01REG_RETR, ARD_500US |
                        ( REPEAT_CNT & 0x0F ) );         //repeat wait : 250us  repeat:15times 
    L01_WriteSingleReg( L01REG_RF_CH, 40 );             //Initial channel
    L01_WriteSingleReg( L01REG_RF_SETUP, 0x00); //0x06); //发射功率
...
void L01_Config_Len17( void ) 
{
    L01_CSN_LOW( );
    L01_WriteSingleReg( L01REG_RX_PW_P0, 17 );	
    L01_CE_HIGH( );
}

void L01_Config_Len30( void ) 
{
    L01_CSN_LOW( );
    L01_WriteSingleReg( L01REG_RX_PW_P0, 30 );
    L01_CE_HIGH( );
}

void RF_IRQ( void )	reentrant interrupt INTERRUPT_RFIRQ 
{
	...
	{ 
		L01_ReadRXPayload(rxbuf);
	
		CLKCTRL = 0x28;    //reset clk
